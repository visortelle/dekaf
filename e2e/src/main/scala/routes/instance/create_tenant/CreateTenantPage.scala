...
case class CreateTenantPage(root: Locator):
    ...
    val adminRoleListInput: ListInput = ListInput(root.getByTestId("admin-roles-input"))
    val clustersSelectListInput: ListInput = ListInput(root.getByTestId("clusters-input"))
    ...
    def addAdminRole(role: String): Unit = adminRoleListInput.addItem(role)
    def addCluster(cluster: String): Unit = clustersSelectListInput.addItem(cluster)
    def create(): Unit =
        createButton.click()
