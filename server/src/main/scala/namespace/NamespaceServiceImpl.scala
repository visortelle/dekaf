...
class NamespaceServiceImpl extends NamespaceServiceGrpc.NamespaceService:
    ...
    override def getNamespaces(request: GetNamespacesRequest): Future[GetNamespacesResponse] =
        ...
        val namespaces = try {
            ...
            case err =>
                ...
                return Future.successful(pb.GetNamespacesResponse(status = Some(status)))
        }
    ...
    override def grantPermissions(request: GrantPermissionsRequest): Future[GrantPermissionsResponse] =
        logger.debug(s"Graning permissions for namespace: ${request.namespace}")
