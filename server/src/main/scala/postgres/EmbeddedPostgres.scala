...
object EmbeddedPostgres:
    private def initEmptyDataDir(embeddedPostgresConfig: EmbeddedPostgresConfig): Task[Unit] = for {
        ...
        p2 <- Command(
            "psql",
            "--host",
            embeddedPostgresConfig.host,
            "--port",
            embeddedPostgresConfig.port.toString,
            "--username",
            embeddedPostgresConfig.user,
            "--dbname",
            "postgres",
            "--command",
            s"CREATE DATABASE ${embeddedPostgresConfig.database}"
        ).env(Map("PGPASSWORD" -> embeddedPostgresConfig.password)).inheritIO.run
        _ <- p2.successfulExitCode.ensuring(startPostgresResult.process.kill.orElseSucceed(()))
    } yield ()
    ...
    private def preStart: Task[Unit] = for {
