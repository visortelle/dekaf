  ...
  const { data: initialValue, error: initialValueError } = useSWR(
    ...
    async () => {
      ...
      if (res.getStatus()?.getCode() !== Code.OK) {
        notifyError(`Unable to get max subscriptions per topic: ${res.getStatus()?.getMessage()}`);
        return;
      }
      ...
      switch (res.getMaxSubscriptionsPerTopicCase()) {
        ...
        case pb.GetMaxSubscriptionsPerTopicResponse.MaxSubscriptionsPerTopicCase.SPECIFIED: {
          ...
          if (maxSubscriptionsPerTopic === 0) {
            ...
            initialValue = { type: 'specified-for-this-namespace', maxSubscriptionsPerTopic };
          }
          ...
          <>
              ...
              <Select<PolicyValue['type']>
                ...
                onChange={(v) => {
                  switch (v) {
                    ...
                    case 'specified-for-this-namespace': onChange({ type: 'specified-for-this-namespace', maxSubscriptionsPerTopic: 1 }); break;
                  }
                }}
                value={value.type}
              />
            ...
            {value.type === 'specified-for-this-namespace' && (
              <Input
                ...
                onChange={v => onChange({ type: 'specified-for-this-namespace', maxSubscriptionsPerTopic: parseInt(v) })}
              />
            )}
          </>
        );
