...
describe("takeMessageFields", () => {
  it("should take only fields marked as 'active'", () => {
    Array.from(Array(20)).forEach(() => {
      const messages: MessageDescriptor[] = Array.from(Array(10)).map(() => genMessageDescriptor());
      ...
      const want = messages.map((message) => {
        const fieldKeysToTake = config.fields.filter((field) => field.isActive).map<keyof MessageDescriptor>((field) => field.id);

        return fieldKeysToTake.reduce<Partial<MessageDescriptor>>((acc, fieldKey) => {
...
describe("encodeBigArrayAsJsonBytes", () => {
  it("serialize array of objects as minified JSON", () => {
    const got = encodeBigArrayAsJsonBytes(
      [
        { name: "John", age: 42 },
        { name: "Jane", age: 42 },
      ],
  ...
  it("handle big arrays", () => {
    ...
    const got = encodeBigArrayAsJsonBytes(bigArray, JSON.stringify);
    expect(got.length).toBeGreaterThanOrEqual(megabytesCount * bytesInKb);
  });

  it("return [] if no data to serialize", () => {
    const got = encodeBigArrayAsJsonBytes([], JSON.stringify);
...
describe("splitMessagesToChunks", () => {
  ...
  it.each(dataset)(`split messages to chunks, limited by chunk size in bytes`, ({ messages, maxBytesPerChunk, expectedChunksCount }) => {
    ...
    const messagesInChunks = chunks.reduce<PartialMessageDescriptor[]>((acc, chunk) => acc.concat(chunk.messages), []);
    ...
    let expectedMinChunksCount = Math.ceil(bytesInMessages / maxBytesPerChunk);
