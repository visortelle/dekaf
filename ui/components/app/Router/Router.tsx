...
import {
  ...
  SchemaResource,
  SubscriptionResource, TenantResource, TopicResource
...
const ResourceExistsOr404: React.FC<{
  ...
  const { data: isResourceExists, isLoading: isResourceExistsLoading, error: isResourceExistsError } = useSWR(
    swrKeys.pulsar.customApi.resourceFqnCheck._({
      tenant: tenant || "",
      namespace: namespace || "",
      topic: topic || "",
  ...
  return !isResourceExistsLoading ?
    isResourceExists ?
      props.children
      : <PageNotFound/>
    : (tenant && namespace && topic && topicPersistency) ? // to avoid topic auto-creation
      <></>
      : props.children;
}
