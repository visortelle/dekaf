...
object ProcessorPlan:
    ...
    def make[A, B](
        ...
        for {
            ...
            workers <- ZIO.foreachPar(producerPlans zip consumerPlans) { case (producerPlan, consumerPlan) =>
                ZIO.succeed(
                    ProcessorWorker[A, B](
                        consumingTopicPlan,
                        producingTopicPlan,
...
object ProcessorPlanGenerator:
    def make[A, B](
        ...
        mkSubscriptionPlan: ProcessorIndex => Task[SubscriptionPlan] = _ =>
            SubscriptionPlan.make(SubscriptionType.Shared),
        mkWorkerCount: ProcessorIndex => Int = _ => 1,
        mkWorkerConsumerName: ProcessorIndex => ConsumerIndex => WorkerName = processorIndex =>
            consumerIndex => s"dekaf-processor-worker-$processorIndex-$consumerIndex",
        mkWorkerProducerName: ProcessorIndex => ProducerIndex => WorkerName = processorIndex =>
            producerIndex => s"dekaf-processor-worker-$processorIndex-$producerIndex",
        mkMessageListenerBuilder: ProcessorIndex => ProcessorMessageListenerBuilder[A, B] = _ =>
            (worker: ProcessorWorker[A, B], producer: Producer[B]) =>
                (consumer: Consumer[A], msg: PulsarMessage[A]) => consumer.acknowledge(msg)
        ...
        for {
            ...
            _ <- ZIO.foreachParDiscard(processorPlan.workers) { worker =>
                for {
                    ...
                    _ <- ZIO.logInfo(
                        s"Started processor worker with consumer: \"${worker.consumerPlan.name}\" and producer: \"${worker.producerPlan.name}\""
                    )
                } yield ()
            }
        } yield ()
