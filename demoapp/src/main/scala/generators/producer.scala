...
object ProducerPlanExecutor:
        ...
        for {
            ...
            _ <- producerPlan.messageIndex
                .update { messageIndex =>
                    ...
                    message.properties.foreach(properties => messageBuilder.properties(properties.asJava))
    ...
    def startProduce(topic: TopicPlan): Task[Unit] =
        ZIO.foreachParDiscard(topic.producers.values)(producerPlan => startProduce(producerPlan, topic))
    ...
    def startMultiTopicProducer(producerPlan: ProducerPlan, topicPlans: List[TopicPlan]): Task[Unit] =
        for {
            ...
            _ <- ZIO.logInfo(
                s"Started producers ${producerPlan.name} for topics ${topicPlans.map(_.name).mkString(", ")}"
            )
            _ <- producerPlan.messageIndex
                .update { messageIndex =>
                    val message = producerPlan.mkMessage(messageIndex)
