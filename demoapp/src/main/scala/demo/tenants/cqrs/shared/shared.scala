...
object DemoAppTopicConfig:
    ...
    val workersAmount: Int =
        config.demoAppConfig.flatMap(_.workersAmount).getOrElse(faker.number().numberBetween(1, 10))
    ...
    def logDemoAppConfig = for {
        _ <- ZIO.logInfo(
            "Overloaded topic schedule time: " + DemoAppTopicConfig.ScheduleTime.overloadedTopic / Math.pow(10, 6)
        )
        _ <- ZIO.logInfo(
            "Heavily loaded topic schedule time: " + DemoAppTopicConfig.ScheduleTime.heavilyLoadedTopic / Math.pow(
                10,
                6
            )
        )
        _ <- ZIO.logInfo(
            "Moderately loaded topic schedule time: " + DemoAppTopicConfig.ScheduleTime.moderatelyLoadedTopic / Math
                .pow(10, 6)
        )
        _ <- ZIO.logInfo(
            "Lightly loaded topic schedule time: " + DemoAppTopicConfig.ScheduleTime.lightlyLoadedTopic / Math.pow(
                10,
                6
            )
        )
        _ <- ZIO.logInfo(
            "Overloaded topic subscription amount: " + DemoAppTopicConfig.SubscriptionAmount.overloadedTopic.toString
        )
        _ <- ZIO.logInfo(
            "Heavily loaded topic subscription amount: " + DemoAppTopicConfig.SubscriptionAmount.heavilyLoadedTopic.toString
        )
        _ <- ZIO.logInfo(
            "Moderately loaded topic subscription amount: " + DemoAppTopicConfig.SubscriptionAmount.moderatelyLoadedTopic.toString
        )
        _ <- ZIO.logInfo(
            "Lightly loaded topic subscription amount: " + DemoAppTopicConfig.SubscriptionAmount.lightlyLoadedTopic.toString
        )
        _ <- ZIO.logInfo(
            "Overloaded topic consumer amount: " + DemoAppTopicConfig.ConsumerAmount.overloadedTopic.toString
        )
        _ <- ZIO.logInfo(
            "Heavily loaded topic consumer amount: " + DemoAppTopicConfig.ConsumerAmount.heavilyLoadedTopic.toString
        )
        _ <- ZIO.logInfo(
            "Moderately loaded topic consumer amount: " + DemoAppTopicConfig.ConsumerAmount.moderatelyLoadedTopic.toString
        )
        _ <- ZIO.logInfo(
            "Lightly loaded topic consumer amount: " + DemoAppTopicConfig.ConsumerAmount.lightlyLoadedTopic.toString
        )
