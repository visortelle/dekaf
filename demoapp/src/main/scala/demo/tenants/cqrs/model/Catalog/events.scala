...
object CatalogDescriptionChanged
    extends Randomizable[CatalogDescriptionChanged]
    with Schemable[CatalogDescriptionChanged]:
...
case class CatalogItemIncreased(catalogId: UUID, itemId: UUID, inventoryId: UUID, quantity: Int, version: Long)
    extends Event
