package demo.tenants.cqrs.model.Dto

import _root_.demo.tenants.cqrs.shared.faker
import demo.tenants.cqrs.*
...
object Product extends Randomizable[Product]:
    override def random: Product = Product(
        ...
        sku =
            s"${faker.lorem().characters(2, 4).toUpperCase}/${faker.number().randomNumber(2, true)}/${faker.lorem().characters(2, 4).toUpperCase}"
    )
...
object Profile extends Randomizable[Profile]:
    override def random: Profile = Profile(
        ...
        email = faker.internet().emailAddress(),
        birthdate =
            if faker.bool().bool() then Some(faker.date().birthday().toInstant.atOffset(ZoneOffset.UTC))
            else None,
